...
- **Capacity/conflict checks are application-level, not DB-enforced.** Both RSVP
  capacity and resource-booking conflict detection use a check-then-insert
  pattern (`SELECT count/overlaps` → `if OK, INSERT`). Under concurrent requests
  (two people grabbing the last RSVP slot at the same instant, or double-booking
  a resource simultaneously), both could theoretically slip through since there's
  no DB-level unique constraint or pessimistic lock backing the check. Fixing
  this properly means either a `SELECT ... FOR UPDATE` pessimistic lock in the
  service methods, or a serializable transaction isolation level — worth
  mentioning if asked about this project in an interview, since "how would you
  handle this at scale" is exactly the kind of follow-up this invites.
- No endpoint to promote a user to `ORGANIZER`/`ADMIN` — currently done via a
  manual SQL `UPDATE` (see Getting Started). A real admin-only "manage users"
  endpoint would be a natural next addition.
- No refresh-token flow — JWTs are long-lived (24h) with no silent renewal or
  auto-redirect-to-login on expiry in the frontend.
